                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 p-3 xl:px-10 gap-y-8 gap-x-12 2xl:gap-x-28">
                    <div class="w-full">
                        <p tabindex="0" class="focus:outline-none text-xs md:text-sm font-medium leading-none text-gray-500 dark:text-gray-300 uppercase">
                            <a href="https://<?php if(isset($_SERVER['HTTP_ACCEPT_LANGUAGE'])){echo substr($_SERVER['HTTP_ACCEPT_LANGUAGE'], 0, 2).".";}?>wikipedia.org/wiki/{{$species->sci_name}}" class="hover:text-green-300 hover:bg-blue-800 hover:rounded-2xl hover:overflow-hidden hover:z-0" target="_blank">{{ $species->sci_name}}</a>
                        </p>
                        <p tabindex="0" class="focus:outline-none text-lg sm:text-xl md:text-2xl lg:text-3xl font-bold leading-3 text-gray-800 dark:text-gray-300 mt-3 md:mt-5">{{ $species->com_name }}</p>
                    </div>
                    <div class="w-full">
